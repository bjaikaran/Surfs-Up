...
def precipitation():
    ...
    CY = dt.date.today()
    prcp_str = "SELECT date, prcp FROM measurements WHERE date >'"+str(PY)+"' AND date <='"+str(CY)+"'  ORDER BY date ASC"
...
def stations():
    stat_str = "SELECT station FROM stations"
...
def temp_monthly():
    CY = dt.date.today()
    PY = dt.date.today() - dt.timedelta(days=365)
    tobs_str= "SELECT date, tobs FROM measurements WHERE date >'"+str(PY)+"' AND date <='"+str(CY)+"'  ORDER BY date ASC"
...
def stats(start=None):
    no_e_str = "SELECT MIN(tobs) Min, AVG(tobs) Avg, MAX(tobs) Max FROM measurements WHERE date >'"+start+"' ORDER BY date ASC"
...
def stat_range(start=None, end=None):
    se_str = "SELECT MIN(tobs) Min, AVG(tobs) Avg, MAX(tobs) Max FROM measurements WHERE date >'" + start + "' AND date <='" + end + "' ORDER BY date ASC"
